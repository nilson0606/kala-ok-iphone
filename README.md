...
新版錄音保存成品、獨立麥克風音軌與當次評分／播放時間快照（只在 IndexedDB）。頁面下方可填總延時：正值提前歌唱者，負值延後；重新評分保留原設定及遮罩，重新混音另存 WAV、不覆蓋原檔。伴奏及和音同步、共用音量。原始與合成錄音均可透過受 Origin／token 保護的 localhost `/recordings/mp3`，由 FFmpeg 轉 192 kbps MP3 下載；不傳雲端，暫存檔自動清除。舊錄音缺乏來源資料時只開放轉檔。更新本機工具後需重啟。
