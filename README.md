...
6. 基準自動保存在這台電腦；同一影片及分析範圍下次直接載入，不用再分離。展開「本機歌曲庫」可載入或刪除，每筆顯示容量；15／30／60 秒與完整歌曲分開保存。
...
- 網頁啟動的工作先在 `.runtime/jobs/<id>` 暫存下載及分離音檔；建立數值基準後立即刪除音訊，數值基準保存到 `.runtime/library/<影片_範圍_版本>/reference.json`；勾選試聽時同時保存 128 kbps MP3 人聲與伴奏。原始混音、大型 WAV 及磁碟工作目錄會刪除。
- 唱完／取消／切換歌曲會釋放本次記憶體工作，已保存歌曲庫保留到手動刪除。網頁關閉時也嘗試清理本次工作；未送達時，持續執行的工具約 15 分鐘閒置後清理暫存工作。突然斷電時，殘留暫存由工具下次啟動／定期清理處理。歌曲庫不會被閒置清理。
