...
`recording-store.mjs` 將每秒音訊 chunk 與 metadata 以 IndexedDB transaction 原子寫入，完整結束標記 complete；崩潰時保留已提交片段。每段有獨立 ID，不覆蓋重唱前的錄音。空間或寫入失敗時保留記憶體音訊並提供下載；錄音成品與來源資料在完整結束後搬入歌曲庫下的「錄音」目錄，localStorage 分數仍獨立。分數勾選依清單位置區分同名紀錄，刪除前核對已渲染快照，避免另一分頁修改後錯刪。
...
新版錄音保存成品、獨立麥克風音軌與當次評分／播放時間快照（錄製中暫存 IndexedDB，停止後搬入歌曲庫的「錄音」子目錄）。頁面下方可填總延時：正值提前歌唱者，負值延後；重新評分保留原設定及遮罩，重新混音另存 WAV、不覆蓋原檔。伴奏及和音同步、共用音量。原始與合成錄音均可透過受 Origin／token 保護的 localhost `/recordings/mp3`，由 FFmpeg 轉 192 kbps MP3 下載；不傳雲端，暫存檔自動清除。舊錄音缺乏來源資料時只開放轉檔。更新本機工具後需重啟。

評分取樣以最接近 `i × step` 基準時間的觀測為準，不再以每個 100 ms 格子最後一次觀測覆蓋，避免換音時出現時間偏差。此規則同時用於即時評分與錄音後重評；權重、難度與八度規則不變，舊基準不需重新分離。

錄音後重評改從保存的乾淨歌聲解碼，由本機 Web Worker 重新擷取置中音高（20 ms hop）。歌聲先按延時移動，再依重混 segments 映射到歌曲時間，避免與即時播放器時間戳混用；分析緩存在該錄音 metadata。顯示同音檔 0 ms 的進拍分數供比較，沒有預設最佳補償或調分。


### 錄音檔案保存與搬存

`recording-archive.mjs` 透過既有 Origin／token 保護的 localhost API，保存到目前歌曲庫的 `錄音/<recording UUID>/`。`metadata.json` 保存原始基準、播放位置、音高資料和重評结果；`mix.webm`／`mix.wav`、`voice.webm`（依錄音 MIME）保存音檔，MP3 轉檔另留 `export.mp3`。原始來源與重新合成成品各自獨立。

瀏覽器每秒仍寫入 IndexedDB，完整結束後以分段封包搬存。先驗證音檔大小、完成暫存目錄與原子提交，再移除瀏覽器副本；失敗保留原檔供重試。開啟／重新整理錄音時自動搬存既有完整錄音；Chrome 和 Edge 需各開一次以搬出各自舊資料。已搬存資料由本機工具共用，重評更新磁碟 metadata，不改原錄音基準。

清單提供單筆與全部刪除，先確認再刪除錄音及後處理來源，不涉及歌曲分離音軌或基準。刪除記號避免尚未更新的瀏覽器舊副本把錄音搬回。目錄只接受 UUID 與本工具 manifest；檔案讀寫限制在目前歌曲庫，切換位置後過期操作會被拒絕。測試涵蓋搬存完整性、失敗回復、跨瀏覽器、重評／MP3 保存、刪除與全刪除。
