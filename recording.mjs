...
export function createSingerRecorder(options) {
  ...
  async function render() {
    ...
    for (const row of rows) {
      ...
      label.textContent = row.title + recordingSofteningSuffix(row) + (recordingDelaySuffix(row) ? ' '+recordingDelaySuffix(row) : '');
  ...
  function download(blob, meta) {
    ...
    link.href = url; link.download = `${meta.title.replace(/[\\/:*?"<>|]/g,'_').slice(0,80)}-${new Date(meta.created).toISOString().replace(/[:.]/g,'-')}${recordingSofteningSuffix(meta)}${recordingDelaySuffix(meta)}.${meta.mime.includes('wav') ? 'wav' : meta.mime.includes('mp4') ? 'm4a' : 'webm'}`;
